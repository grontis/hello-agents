...
**Handoff rule (read before invoking):** This is one serial stage of a five-stage pipeline (architect → coder → code-reviewer → qa → pr). Run only the qa subagent this turn. When it finishes, present its report and verdict and **STOP** — do not invoke `/implement` or `/review-code` in the same turn, even if QA passes, and never launch them in parallel with this stage. Wait for explicit user approval before any follow-up stage.
